#Inclusion Criterion increase beta_3
...
beta.star = matrix(c(-1.2, log(1.3), log(1.3), log(2.8)),nrow = d.X+1) # beta.star
...
for(sim in 1:no.of.simulations)
{
  ...
  result.same = GENMETA(studies, reference, model, initial_val = c(-1.2, log(1.3), log(1.3), log(2.8)))
...
write.csv(Reduce("+", corr.data.m1) / length(corr.data.m1), file = "Simulation_14_study_1_corr.csv")
write.csv(Reduce("+", corr.data.m2) / length(corr.data.m2), file = "Simulation_14_study_2_corr.csv")
write.csv(Reduce("+", corr.data.m3) / length(corr.data.m3), file = "Simulation_14_study_3_corr.csv")
