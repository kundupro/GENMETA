...
for(sim in 1:no.of.simulations)
{
  ...
  X.rf = exp(mvrnorm(n = n, mu.1, Sigma.1))
  
  
  # Generate data set 1. m1 means model 1.
  X.m1 = exp(mvrnorm(n = n1, mu.1, Sigma.1)) # Generate the covariates.
...
write.csv(result, file = "Simulation_10.csv", row.names = F)

write.csv(Reduce("+", corr.data.m1) / length(corr.data.m1), file = "Simulation_10_study_1_corr.csv")
write.csv(Reduce("+", corr.data.m2) / length(corr.data.m2), file = "Simulation_10_study_2_corr.csv")
write.csv(Reduce("+", corr.data.m3) / length(corr.data.m3), file = "Simulation_10_study_3_corr.csv")
