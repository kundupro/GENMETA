#Equal means and variances across studies, different correlations across studies but ref is one of them(second study)
...
for(sim in 1:no.of.simulations)
{
  ...
  X.rf = exp(mvrnorm(n = n, mu.1, Sigma.2))
...
write.csv(result, file = "Simulation_7.csv", row.names = F)
