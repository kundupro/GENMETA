...
for(sim in 1:no.of.simulations)
{
  ...
  X.rf = exp(mvrnorm(n = n, mu, Sigma))
  #X.rf = exp(X.rf)
 # X.rf = scale(X.rf)
  
  
  # Generate data set 1. m1 means model 1.
  X.m1 = exp(mvrnorm(n = n1, mu, Sigma)) # Generate the covariates.
  #X.m1 = exp(X.m1)
  ...
  X.m2 = exp(mvrnorm(n = n2, mu, Sigma))
  #X.m2 = exp(X.m2)
  ...
  X.m3 = exp(mvrnorm(n = n3, mu, Sigma))
 # X.m3 = exp(X.m3)
