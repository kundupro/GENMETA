#Equal means and variances across studies, same correlations across studies but ref is different(third study)
...
for(sim in 1:no.of.simulations)
{
  ...
  X.m1 = exp(mvrnorm(n = n1, mu.1, Sigma.2)) # Generate the covariates.
  ...
  X.m3 = exp(mvrnorm(n = n3, mu.3, Sigma.2))
...
write.csv(result, file = "Simulation_4.csv", row.names = F)
