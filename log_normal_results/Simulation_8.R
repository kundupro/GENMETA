#Equal means and variances across studies, different correlations across studies but ref is one of them(third study)
...
for(sim in 1:no.of.simulations)
{
  ...
  X.rf = exp(mvrnorm(n = n, mu.1, Sigma.3))
...
write.csv(result, file = "Simulation_8.csv", row.names = F)
