...
#' @param control an optional list containing the epsilon (numeric) and maxiter (numeric) needed for convergence of the algorithm. Default epsilon and maximum iterations are 1e-06 and 1000, respectively. For creating a control argument for GENMETA, see \code{\link[GENMETA]{GENMETA.control}}.
...
#' @seealso \code{\link[GENMETA]{GENMETA.summary}}, \code{\link[GENMETA]{study_info}}, \code{\link[GENMETA]{reference_data}}, \code{\link[GENMETA]{GENMETA.plot}}.
#' @examples
#' # This example shows the GENMETA implementation on a simulated data set for logistic regression
#' data(reference_data)
#' head(reference_data)
#' data(study_info)
#' head(study_info)
...
GENMETA <- function(study_info, ref_dat, model, variable_intercepts=FALSE, initial_val=NULL, control = list(...))
{
 ...
 #  }
  ...
  #   maxit <- control[[2]]
  # }
  call_GENMETA <- match.call()
  ## Checking if the control argument is missing
  if(missing(control))
  {
    control <- GENMETA.control()
  }else{
    control <- do.call("GENMETA.control", control)
  }
  threshold <- control[[1]]
  maxit <- control[[2]]
    ...
    if(error_1 == 0 && error_2 == 0 && error_3 == 0 && error_4 == 0)
    {
      ...
      if(length(initial_val) == 0)
      {
        ...
        if(model_optim == "linear")
        {
            ...
            linear_result <- list("Est.coeff" = beta_old, "Est.var.cov" = asy_var_opt, "Res.var" = disp_max_old, "iter" = no_of_iter, "call" = call_GENMETA)
        ...
        if(model_optim == "logistic")
        {
                ...
                logistic_result <- list("Est.coeff" = beta_initial, "Est.var.cov" = asy_var_beta_converged, "Res.var" = NA, "iter" = total_iter, "call" = call_GENMETA)
